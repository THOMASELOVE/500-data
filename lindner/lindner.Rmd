...
Information on the lindner dataset can be found [here](https://www.rdocumentation.org/packages/MatchLinReg/versions/0.7.0/topics/lindner) ^1,2^
...
The mean `cardbill` was higher in the treated population. A larger percentage of controls did not survive through 6 months.
...
- Across the entire sample, the mean (\$16,127 vs. \$14,614) and median (\$12,944 vs. \$10,423) cardiac care costs were higher in treated individuals than non-treated participants. 
```{r}
...
Treated individuals were estimated to spend `r round(unadjust_quant_outcome_tidy$estimate,2)` (95%CI `r round(unadjust_quant_outcome_tidy$conf.low,2)`, `r round(unadjust_quant_outcome_tidy$conf.high,2)`) more dollars than non-treated individuals. 
...
The predicted odds of being alive after six months in treated individuals was  `r round(unadjust_binary_outcome_tidy$estimate,2)` times higher than the odds that a  non-treated individual would be alive after six months (95%CI `r round(unadjust_binary_outcome_tidy$conf.low,2)`, `r round(unadjust_binary_outcome_tidy$conf.high,2)`).

# Task 2: Fitting the propensity score model

Predict treatment status based on available covariates. We're not worried about overfitting (including too many covariates) when calculating the propensity scores.
...
Above we can see there are no propensity scores equal to, or very close to, 0 or 1. 
...
Both plots demonstrate good overlap, suggesting a propensity score analysis may be appropriate.
...
- Match 1 treated patient to 1 control patient (so we'll have "left over" treated patients not included in the sample)
...
- Note: all controls are used in this sample. However, only 298/698 treated individuals were used. 
...
We'll use a mixed model to estimate the effect of the treatment on `cardbill`.

- The matches will be treated as a random effect (syntax "(1| matches.f)"

- Treatment group will be treated as a fixed effect.

- Restricted maximum likelihood (REML) used to estimate coefficient values.
...
Treated individuals were estimated to spend \$`r round(tidy_mixed_matched$estimate,2)` less (95%CI `r round(tidy_mixed_matched$conf.low,2)`, `r round(tidy_mixed_matched$conf.high,2)`) than non-treated individuals. As this result is not significant at an $\alpha$ of 0.05, a sensitivity analysis on the Quantitative outcome will not make sense.
...
In treated individuals, the mean `cardbill` was lower within the matched sample than the entire sample (note the mean within the control group was the same as every control participant is in the matched sample. The mean changed in the treated group as only 298/698 treated patients are in the matched sample). This is somewhat of a sanity check to assess if the mixed model results make sense. It looks like they do.
...
- To reiterate, as we matched with replacement, and there were less control participants than treated participants, some control participants were matched multiple times.
...
- It definitely looks better than the 1:1 matching without replacement.
...
The odds of being alive after six months were `r round(tidy_binary_outcome_adjusted_rep$estimate,2)` times higher in treated individuals than non-treated individuals (95%CI `r round(tidy_binary_outcome_adjusted_rep$conf.low,2)`, `r round(tidy_binary_outcome_adjusted_rep$conf.high,2)`)
...
Only 20 control individuals were contained in the largest quintile. This is seems low. 
...
Dr. Love's function to calculate the standardizes differences is utilized below.
...
All under 50. Not great, but OK. For comparison, the original Rubin's Rule 1 value was 61.87.
...
The odds of being alive after 6 months was `r round(strat.result2$estimate, 2)` times higher (95%CI `r round(strat.result2$conf.low, 2)`, `r round(strat.result2$conf.high, 2)`) in treated individuals than non-treated individuals. 
...
**Estimate (95%CI)** `r round(wt_ate_results1$estimate, 2)` (`r round(wt_ate_results1$conf.low,2)`, `r round(wt_ate_results1$conf.high,2)`)
...
**Estimate/OR (95%CI)** `r round(wt_ate_results2$estimate,2)` (`r round(wt_ate_results2$conf.low,2)`, `r round(wt_ate_results2$conf.high,2)`)
...
**Estimate (95%CI)** `r round(wt_twangatt_results1$estimate,2)` (`r round(wt_twangatt_results1$conf.low,2)`, `r round(wt_twangatt_results1$conf.high,2)`)
...
**Estimate (95%CI)** `r round(wt_twangatt_results2$estimate,2)` (`r round(wt_twangatt_results2$conf.low,2)`, `r round(wt_twangatt_results2$conf.high,2)`)

# Task 11: After direct adjustment with linear PS

Here we'll directly adjust for the linear PS by including it as a covariate in the model.
...
**Estimate (95%CI)** `r round(adj_out1$estimate,2)` (`r round(adj_out1$conf.low,2)`, `r round(adj_out1$conf.high,2)`)
...
**Estimate/OR (95%CI)** `r round(adj_out2$estimate,2)` (`r round(adj_out2$conf.low,2)`, `r round(adj_out2$conf.high,2)`)
...
**Estimate (95%CI)** `r round(dr_att_out1$estimate,2)` (`r round(dr_att_out1$conf.low,2)`, `r round(dr_att_out1$conf.high,2)`)
...
**Estimate (95%CI)** `r round(dr_ate_out1$estimate,2)` (`r round(dr_ate_out1$conf.low,2)`, `r round(dr_ate_out1$conf.high,2)`)
...
**Estimate (95%CI)** `r round(dr_twangatt_out1$estimate,2)` (`r round(dr_twangatt_out1$conf.low,2)`, `r round(dr_twangatt_out1$conf.high,2)`)
...
**Estimate (95%CI)** `r round(dr_att_out2$estimate,2)` (`r round(dr_att_out2$conf.low,2)`, `r round(dr_att_out2$conf.high,2)`)
...
**Estimate (95%CI)** `r round(dr_ate_out2$estimate,2)` (`r round(dr_ate_out2$conf.low,2)`, `r round(dr_ate_out2$conf.high,2)`)
...
**Estimate (95%CI)** `r round(dr_twangatt_out2$estimate,2)` (`r round(dr_twangatt_out2$conf.low,2)`, `r round(dr_twangatt_out2$conf.high,2)`)
